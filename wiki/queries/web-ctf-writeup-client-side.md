---
title: Web CTF Writeup — 클라이언트 사이드/XSS/CSP
created: 2026-06-14
updated: 2026-06-21
type: query
tags: [ctf, web, research, writeup, xss, csp, client-side, browser-history, bookmarklet, wasm, javascript, source-analysis]
sources: [https://blog.hokyun.dev/posts/google-ctf-2024-quals-writeup/, https://blog.drstra.in/posts/assn-wu/, https://blog.s1r1us.ninja/CTF/IuseBing, https://blog.arkark.dev/2023/12/28/seccon-finals, raw/articles/20260613_web-ctf-writeup-curated.md, https://ctftime.org/writeup/27367, https://github.com/noamgariani11/picoCTF-2024-Writeup/blob/main/Web%20Exploitation/Bookmarklet.md, https://medium.com/@rwsimpson99/picoctf-local-authority-9026cd92436c, https://github.com/noamgariani11/picoCTF-2024-Writeup/blob/main/Web%20Exploitation/WebDecode.md, https://ctftime.org/writeup/27181, https://github.com/Dvd848/CTFs/blob/master/2021_picoCTF/Some_Assembly_Required_3.md, https://github.com/xpinked/ctf-writeups/blob/master/noxCTF18/Web/HiddenDOM/Solution.md, https://github.com/Cajac/picoCTF-Writeups/blob/main/picoCTF_2024/Web_Exploitation/Unminify.md]
confidence: high
---

# Web CTF Writeup — 클라이언트 사이드/XSS/CSP

> 브라우저 상태, CSP, nonce, window.name, JSONP, XS-Leak, bookmarklet, browser history, WASM을 다루는 분류입니다.

## 참고 URL
- [blog.hokyun.dev](https://blog.hokyun.dev/posts/google-ctf-2024-quals-writeup/)
- [blog.drstra.in](https://blog.drstra.in/posts/assn-wu/)
- [blog.s1r1us.ninja](https://blog.s1r1us.ninja/CTF/IuseBing)
- [blog.arkark.dev](https://blog.arkark.dev/2023/12/28/seccon-finals)
- [raw/articles/20260613_web-ctf-writeup-curated.md](raw/articles/20260613_web-ctf-writeup-curated.md)
- [CTFtime writeup](https://ctftime.org/writeup/27367)
- [Original writeup](https://github.com/noamgariani11/picoCTF-2024-Writeup/blob/main/Web%20Exploitation/Bookmarklet.md)
- [medium.com](https://medium.com/@rwsimpson99/picoctf-local-authority-9026cd92436c)
- [Original writeup](https://github.com/noamgariani11/picoCTF-2024-Writeup/blob/main/Web%20Exploitation/WebDecode.md)
- [CTFtime writeup](https://ctftime.org/writeup/27181)
- [Original writeup](https://github.com/Dvd848/CTFs/blob/master/2021_picoCTF/Some_Assembly_Required_3.md)
- [Original writeup](https://github.com/xpinked/ctf-writeups/blob/master/noxCTF18/Web/HiddenDOM/Solution.md)
- [Original writeup](https://github.com/Cajac/picoCTF-Writeups/blob/main/picoCTF_2024/Web_Exploitation/Unminify.md)


## 1. 핵심 요약
- 이 분류는 **브라우저가 실제로 실행하는 것**과 **개발자가 안전하다고 믿은 것**의 차이를 다룹니다.
- `XSS` 자체보다 `CSP 우회`, `nonce leak`, `window.name`, `prototype pollution`, `history state`, `bookmarklet` 체인이 핵심인 경우가 많습니다.
- `web-inspector`, `source inspection`, `csrf`, `cors-misconfig`, `xss`와 함께 읽으면 좋습니다.

연결 개념: [[xss]], [[csrf]], [[web-inspector-ctf-patterns]], [[websocket-message-tampering-ctf-patterns]], [[cors-misconfig]], [[ssrf-defense]], [[csp-bypass-ctf-patterns]], [[source-inspection-minification-ctf-patterns]]

## 2. 대표 writeup

||| 문제 | 출처 | 핵심 아이디어 |
|||------|------|---------------|
|| `Ancient History` | picoCTF 2021 | browser history back으로 이전 상태에서 flag 회수 |
|| `Bookmarklet` | picoCTF 2024 | bookmarklet 실행, browser JavaScript execution |
|| `Local Authority` | picoCTF | 클라이언트 JS에 숨은 인증 로직과 평문 자격 증명 |
|| `WebDecode` | picoCTF 2024 | hidden attribute/source에서 encoded flag 추출 |
|| `Includes` | picoCTF 2022 | HTML/CSS/JS 정적 파일에 flag 조각 분산 |
|| `Some Assembly Required 3` | picoCTF 2021 | WebAssembly 디컴파일과 XOR key 복원 |
|| `HiddenDOM` | noxCTF18 | obfuscated JS와 hidden parameter로 DOM 규칙 제어 |
|| `Elements` | picoCTF 2024 | XSS + CSP bypass + server-side bot, timing side-channel |
|| `websockfish` | picoCTF 2025 | WebSocket `eval` 메시지 변조로 state tampering |
|| `sappy` | Google CTF 2024 Quals | host 검증 우회 후 메시지 기반 렌더링 조작 |
|| `Another secure store note` | LINE CTF 2023 | CSRF + nonce leak + CSP bypass |
|| `ALL THE LITTLE THINGS` | Google CTF 2020 | prototype pollution + `window.name` + JSONP |
|| `babywaf` | SECCON 2023 Finals | WAF와 JSON 파싱 차이로 우회 |
|| `cgi-2023` | SECCON 2023 Finals | XS-Leak와 SRI를 이용한 오라클 구성 |

## 3. 자주 보이는 패턴
1. `postMessage` 또는 `window.name`에 상태를 저장함
2. `document.domain`, `iframe`, `nonce`가 공격면이 됨
3. CSP가 엄격하지만 nonce/리포트 채널이 존재함
4. 브라우저와 서버의 파서 차이로 `JSON.parse` 오라클이 생김
5. 이미지, 스타일, 리소스 로드가 정보 누출 통로가 됨
6. WebSocket 메시지의 상태값·점수·명령이 서버 로직에 영향을 줌
7. history stack / bookmarklet / hidden script가 같은 client-side attack surface로 묶임

## 4. 읽을 때 확인할 것
- CSP가 어떤 소스를 허용하는지
- nonce가 어디서 생성되고 언제 고정되는지
- 프레임/팝업/리다이렉트가 같은 origin으로 유지되는지
- `window.name`, `localStorage`, 쿠키가 어떤 시점에 읽히는지
- history stack이나 bookmarklet 실행으로 이전 상태/현재 맥락이 노출되는지

## 5. 방어 관점
- CSP는 `base-uri`, `object-src`, `frame-ancestors`까지 함께 봐야 합니다.
- origin 검증은 문자열 비교보다 URL 파서 결과를 기준으로 해야 합니다.
- 클라이언트 상태를 신뢰 경계로 쓰지 않습니다.
- `postMessage` 수신자는 origin 체크를 엄격히 해야 합니다.
- 브라우저 history에 민감정보가 남지 않도록 상태 전환을 설계합니다.

## 6. 추천 다음 읽기
- [[xss]]
- [[csrf]]
- [[web-inspector-ctf-patterns]]
- [[source-inspection-minification-ctf-patterns]]
- [[websocket-message-tampering-ctf-patterns]]
- [[web-ctf-master-checklist]]
